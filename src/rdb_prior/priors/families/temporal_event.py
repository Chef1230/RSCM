...
def bind_temporal_event_plan(
    ...
    for bundle in prior_plan.motif_bundles:
        ...
        population_parameters: tuple[tuple[str, object], ...] = (
            ...
            ("population_source", "temporal_event"),
        ...
        temporal_processes.append(
            TemporalProcessPlan(
                ...
                covariate_column_ids=tuple(
                    column.column_id
                    for column in schema.table(entity_id).columns
                    if column.kind is ColumnKind.FEATURE
                    and prior_plan.semantic_schema.column_role(column.column_id).value
                    in {"static_attribute", "state", "measurement", "amount"}
                ),
                parameters=(
                    ("foreign_key_id", foreign_key_id),
        ...
        for column in schema.table(event_id).columns:
            ...
            column_mechanisms.append(
                ColumnMechanismPlan(
                    column_id=column.column_id,
                    family=_semantic_attribute_family(
                        prior_plan.semantic_schema,
                        column.column_id,
                        parameters.get("semantic_mechanism_weights"),
                        rng,
                    ),
                    parent_column_ids=parent_columns,
                    shared_state_ids=(state_id,),
        ...
        if bundle.relation_mechanism.kind == RelationPriorKind.TREE.value:
            # In a temporal bundle the tree contributes to event intensity;
            # owner-FK assignment remains state-conditioned and is not
            # regenerated by a second relation process.
            if "event_intensity_forest" not in parameters:
                raise ValueError("temporal tree relation lacks event-intensity forest")
        if bundle.relation_mechanism.kind == RelationPriorKind.SCM.value:
            relation_family = parameters.get("relation_scm_family")
            if not isinstance(relation_family, str) or not relation_family.startswith("scm_"):
                raise ValueError("temporal SCM relation lacks a sampled SCM family")
            relations = [
                replace(relation, family=relation_family)
...
def _semantic_attribute_family(
    semantic_schema,
    column_id: str,
    weights_payload: object,
    rng: np.random.Generator,
) -> str:
    """Use private semantic roles to choose the event emission mechanism."""
    role = semantic_schema.column_role(column_id).value
    weights = dict(weights_payload) if isinstance(weights_payload, list) else {}
    attribute_weight = float(weights.get("attribute", 0.33))
    temporal_weight = float(weights.get("temporal", 0.33))
    if role in {"amount", "measurement"}:
        preferred = "cam" if temporal_weight > attribute_weight else "linear"
    elif role in {"state", "outcome", "action_type"}:
        preferred = "linear"
    else:
        preferred = "cam" if attribute_weight < temporal_weight else "linear"
    if rng.random() < 0.15:
        return "cam" if preferred == "linear" else "linear"
    return preferred
